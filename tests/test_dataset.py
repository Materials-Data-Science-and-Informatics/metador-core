...
def test_create_open(tmp_ds_path):
    ...
    with IH5Dataset.open(tmp_ds_path) as ds2:
        ...
        with pytest.raises(ValueError):
            ds2["key"] = 123  # setter should not work
        with pytest.raises(ValueError):
            ds2.commit()
        with pytest.raises(ValueError):
            ds2.discard_patch()
...
def test_missing_userblock_open(tmp_ds_path):
...
def test_broken_userblock_open(tmp_ds_path):
    with IH5Dataset.create(tmp_ds_path) as ds:
        cfile = ds.containers[0]
...
def test_create_patch_commit(tmp_ds_path):
    with IH5Dataset.create(tmp_ds_path) as ds:
        ...
        del ds["bar"]
        ds["bar"] = 1337
        # TODO: overwriting directly without DEL should raise exception, not break assertion!
        ds.commit()
